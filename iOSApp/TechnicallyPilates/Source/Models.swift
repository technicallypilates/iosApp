...
struct Exercise: Identifiable, Codable, Equatable {
    ...
    init(id: UUID = UUID(), name: String, description: String, pose: Pose, duration: TimeInterval, repetitions: Int, instructions: [String]) {
        self.id = id
        self.name = name
        self.description = description
        self.pose = pose
...
struct UserProfile: Identifiable, Codable, Equatable {
    ...
    mutating func addXP(_ amount: Int) {
        ...
        // Level up calculation (example: 1000 XP per level)
        let newLevel = (xp / 1000) + 1
        if newLevel > level {
            level = newLevel
            // You could add level-up achievements here
        }
    }
}

// New structure for XP calculation
struct XPSystem {
    static func calculateXP(accuracy: Double, isFirstTimeHighAccuracy: Bool = false, consecutiveCorrectPoses: Int = 0) -> Int {
        ...
        // Perfect form multiplier
        if accuracy >= 0.95 {
            xp *= 2
        }
        
        // Consecutive correct poses bonus
        if consecutiveCorrectPoses > 0 {
            xp += Int(Double(xp) * (0.1 * Double(consecutiveCorrectPoses)))
        }
        
        // First time high accuracy bonus
